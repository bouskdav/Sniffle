...
def main():
    ...
    aparse.add_argument("--ndjson", action="store_true",
            help="Output one JSON object per received ADV (good for piping into other apps)")
    aparse.add_argument("--odid-only", action="store_true",
            help="In ndjson mode: only emit packets containing ODID service data (0x16, UUID FFFA)")
    ...
    while True:
        try:
            ...
            print_message(msg, args.quiet, args.decode, ndjson=args.ndjson, odid_only=args.odid_only)
        except SourceDone:
            break
...
def print_message(msg, quiet, decode_ad, ndjson=False, odid_only=False):
    if isinstance(msg, PacketMessage):
        if ndjson:
            print_packet_ndjson(msg, odid_only=odid_only)
        else:
...
def _has_odid_service_data(adv_data: bytes) -> bool:
    """
    BLE AD structures: [len][type][data...]
    We want AD type 0x16 (Service Data - 16-bit UUID) containing UUID 0xFFFA (little endian FA FF).
    """
    if not adv_data:
        return False
    i = 0
    ...
    while i + 2 <= n:
        ...
        if i + 1 + L > n:
            break
        t = adv_data[i + 1]
        d = adv_data[i + 2 : i + 1 + L]
        # Service Data 16-bit UUID
        if t == 0x16 and len(d) >= 2:
            if d[0] == 0xFA and d[1] == 0xFF:
                return True
        i += 1 + L
    return False

def _get_phy_fields(dpkt) -> tuple[int | None, int | None]:
    """
    Sniffle message types differ. Try common attribute names; return (prim_phy, sec_phy) or (None,None).
    Values are usually 1=1M, 2=2M, 3=CODED.
    """
    prim = getattr(dpkt, "prim_phy", None)
    sec = getattr(dpkt, "sec_phy", None)
    # Some builds name them PrimaryPHY/SecondaryPHY or similar.
    if prim is None:
        prim = getattr(dpkt, "PrimaryPHY", None)
    if sec is None:
        sec = getattr(dpkt, "SecondaryPHY", None)
    return (prim, sec)

def print_packet_ndjson(dpkt, odid_only=False):
    """
    Emit one JSON record per advertising PDU.
    Only emits advertisements (Adva / ScanRsp / AdvExtInd / etc.).
    """
    if not isinstance(dpkt, (AdvaMessage, AdvDirectIndMessage, ScanRspMessage, AdvExtIndMessage)):
        return

    mac = None
    if getattr(dpkt, "AdvA", None) is not None:
        mac = str_mac(dpkt.AdvA)

    # adv_data is what we want (AD structures bytes)
    adv_data = getattr(dpkt, "adv_data", None)
    if adv_data is None:
        return

    if odid_only and not _has_odid_service_data(adv_data):
        return

    rssi = getattr(dpkt, "rssi", None)
    prim_phy, sec_phy = _get_phy_fields(dpkt)

    rec = {
        "type": dpkt.__class__.__name__,
        "mac": mac,
        "rssi": rssi,
        "prim_phy": prim_phy,
        "sec_phy": sec_phy,
        "adv": hexlify(adv_data).decode("ascii"),
    }
